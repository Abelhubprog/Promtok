...
# Promtok Database Reset Script for Docker
...
echo -e "${CYAN}============================================================${NC}"
echo -e "${CYAN}       Promtok Database Reset Tool (Docker)${NC}"
echo -e "${CYAN}============================================================${NC}"
echo
...
if ! docker ps -a --format '{{.Names}}' | grep -q '^promtok-backend
...
docker cp reset_databases.py promtok-backend:/app/reset_databases.py
...
if docker ps --format '{{.Names}}' | grep -q '^promtok-backend
else
...
; then
    echo -e "${RED}Error: promtok-backend container not found${NC}"
    echo "Please ensure the Promtok application is deployed with:"
    echo "  docker compose up -d"
    exit 1
fi
...
; then
    docker exec promtok-backend rm -f /app/reset_databases.py 2>/dev/null || true
fi
