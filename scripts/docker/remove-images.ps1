...
if (-not $IncludeK8s) {
  $images = $allImages | Where-Object { $img = $_; -not ($k8sPatterns | ForEach-Object { $img -match $_ } | Where-Object { $_ }) }
} else {
  $images = $allImages
}
...
function Get-ImageId([string]$ref) {
  $rows = docker images --format "{{.Repository}}:{{.Tag}} {{.ID}}" | Where-Object { $_ -like "$ref *" }
  if ($rows) { return ($rows -split ' ')[1] } else { return $null }
}

function Get-ContainersFor([string]$ref) {
  docker ps -a --filter "ancestor=$ref" --format "{{.ID}}"
}
...
Write-Host "Removed:    $($removed.Count)"; $removed   | ForEach-Object { Write-Host "  - $_" }
Write-Host "Skipped:    $($skipped.Count)"; $skipped   | ForEach-Object { Write-Host "  - $_" }
Write-Host "Not found:  $($notFound.Count)"; $notFound | ForEach-Object { Write-Host "  - $_" }
Write-Host "Errors:     $($errors.Count)"; $errors     | ForEach-Object { Write-Host "  - $_" }
